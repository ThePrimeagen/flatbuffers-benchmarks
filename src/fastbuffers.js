 ...
 * @param {number} length
 */
flatbuffers.stringDecoder = function stringDecoder(bb, offset, length) {
  var i = 0;
  var result = '';
  ...
  while (i < length) {
    ...
    // Decode UTF-8
    var a = bb.readUint8(offset + i++);
    if (a < 0xC0) {
      codePoint = a;
    } else {
      var b = bb.readUint8(offset + i++);
      if (a < 0xE0) {
        codePoint =
          ((a & 0x1F) << 6) |
          (b & 0x3F);
      } else {
        var c = bb.readUint8(offset + i++);
        if (a < 0xF0) {
          codePoint =
            ((a & 0x0F) << 12) |
            ((b & 0x3F) << 6) |
...
flatbuffers.ByteBuffer.prototype.__string = function(offset, optionalEncoding) {
  ...
  return flatbuffers.stringDecoder(this.bb, offset, length);
};
